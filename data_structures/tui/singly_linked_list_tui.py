...
def build_sll_snapshots(items, visible_indices, item_count):
    snapshots = []
    previous_index = None
    ...
    for item, index in zip(items, visible_indices):
        if previous_index is not None and index - previous_index > 1:
            ...
            snapshots.append({"kind": "gap", "hidden_count": hidden_count})
...
def render_horizontal_sll(cards):
    if not cards:
        return ["(empty)"]
...
def build_horizontal_sll_view(items, available_width: int, scroll_offset: int, anchor_target: int = 2):
    ...
    full_cards = build_sll_cards(build_sll_snapshots(items, list(range(item_count)), item_count))
    ...
    for anchor_count in (anchor_target, 1):
        ...
        if middle_count <= 0:
            visible_indices = list(range(item_count))
            visible_items = list(items)
            render_width = estimate_horizontal_width(
                build_sll_cards(build_sll_snapshots(visible_items, visible_indices, item_count))
            )
        ...
        for middle_len in range(max_middle_len, -1, -1):
            ...
            for item, index in zip(visible_items, visible_indices):
                if isinstance(item, dict) and item.get("kind") == "gap":
                    ...
                    snapshots.extend(build_sll_snapshots([item], [index], item_count))

            cards = build_sll_cards(snapshots)
            render_width = estimate_horizontal_width(cards)
            if render_width <= available_width:
                actual_visible_items = [
                    item
                    for item in visible_items
                    if not (isinstance(item, dict) and item.get("kind") == "gap")
                ]
...
class SinglyLinkedListPanel(Static):
    def refresh_singly_linked_list(self, *, header: str, items, view_top=None, scroll_offset: int = 0) -> None:
        available_width = max(self.size.width - 4, 20)
        view = build_horizontal_sll_view(items, available_width, scroll_offset)
        snapshots = build_sll_snapshots(view["visible_items"], view["visible_indices"], view["item_count"])
        cards = build_sll_cards(snapshots)
        render_width = estimate_horizontal_width(cards)
        ...
        lines = render_horizontal_sll(cards)
...
class SinglyLinkedListTUI(BaseLinearStructureTUI):
    CSS = """
    SinglyLinkedListPanel {
        height: auto;
        min-height: 9;
    ...
    def __init__(self, **kwargs):
        self.demo = SinglyLinkedList()
        super().__init__(**kwargs)

    def compose(self) -> ComposeResult:
        yield Header()
        yield SinglyLinkedListPanel(id="panel")
    ...
    def append_values(self, values) -> None:
        if self.structure_size() + len(values) > self.max_size:
            raise OverflowError(f"Singly linked list overflow: capacity is {self.max_size}")
        for value in values:
    ...
    def handle_structure_command(self, verb: str, arg: str | None, log: RichLog) -> bool:
        ...
        if verb == "/remove-value":
            ...
            try:
                ...
                log.write(
                    f"[green]remove-value({value!r})  {self.show_state_text()}  size={self.structure_size()}[/green]"
                )
    ...
    def _refresh_panel(self) -> None:
        ...
        panel = self.query_one(SinglyLinkedListPanel)
        panel.refresh_singly_linked_list(
            header=header,
    ...
    def _clamp_scroll_offset(self) -> None:
        ...
        view = build_horizontal_sll_view(self.demo.node_snapshots(), available_width, self.scroll_offset)
...
def main() -> None:
    parser = build_linear_parser("Interactive Singly Linked List Demo")
