...
def test_insert_and_display_forward(linked_list):
    insert_items(linked_list, ITEMS)
    assert linked_list.display_forward() == ITEMS


def test_display_backward(linked_list):
    insert_items(linked_list, ITEMS)
    assert linked_list.display_backward() == list(reversed(ITEMS))


def test_remove(linked_list):
    insert_items(linked_list, ITEMS)
    linked_list.remove(REMOVED_ITEM)
    assert linked_list.display_forward() == [
        item for item in ITEMS if item != REMOVED_ITEM
    ]
...
def test_node_snapshots_include_stable_node_labels_and_links(linked_list):
    ...
    snapshots = linked_list.node_snapshots()
    assert snapshots == [
        {"node_id": 1, "node_label": "node-1", "data": 10, "prev_label": "/", "next_label": "node-2"},
        {"node_id": 2, "node_label": "node-2", "data": 20, "prev_label": "node-1", "next_label": "/"},
    ]
...
def test_remove_at_tail(linked_list):
    ...
    assert linked_list.to_list() == ITEMS[:-1]
    assert linked_list.tail.data == ITEMS[-2]
...
def test_remove_by_node_id_tail(linked_list):
    insert_items(linked_list, ITEMS)
    node_id = linked_list.node_snapshots()[-1]["node_id"]
    linked_list.remove_by_node_id(node_id)
    assert linked_list.tail.data == ITEMS[-2]
...
def test_insert_at_tail(linked_list):
    ...
    assert linked_list.tail.data == 99
    assert linked_list.tail.next is None
