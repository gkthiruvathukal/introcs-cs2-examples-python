...
Valid short `<name>` aliases: `bst`, `deque`, `dict`, `dll`, `graph`, `heap`, `list`, `networkx`, `queue`, `set`, `sll`, `stack`, `trie`. App variants: `stack-app`, `queue-app`, `deque-app`, `list-app`, `sll-app`, `dll-app`.
...
- **`stack_app.py`** — standalone, self-contained. Defines its own model, panel widget, and command dispatcher. Does not inherit from the shared base.
...
Each TUI has a single canonical tape file: `demos/<name>.tape`. The `--mode dark|light` flag on `run_vhs.py` injects the appropriate theme and appends `-dark` or `-light` to the output filename. Dark theme: `TokyoNight`. Light theme: `Catppuccin Latte`.
